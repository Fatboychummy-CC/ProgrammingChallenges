...
local function run(input, output)
  -- Read every first number into A, and every second number into a dictionary B
  local A, B = {}, {}

  for line in input:lines() do
    local a, b = line:match("(%d+)%s+(%d+)")
    table.insert(A, tonumber(a))
    B[tonumber(b)] = B[tonumber(b)] and B[tonumber(b)] + 1 or 1
  end

  -- Calculate how many times each value from the A list appears in B, and sum the multiplication of the count and value.
  local sum = 0
  for i = 1, #A do
    sum = sum + A[i] * (B[A[i]] or 0)
  end
