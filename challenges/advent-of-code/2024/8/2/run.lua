...
local function run(input, output)
  ...
  --
  -- Part 2: Antinodes now occur at any point that is perfectly in line with two
  -- antennas of the same frequency. This also means that the antenna location
  -- itself becomes an antinode.
  ...
  for _, pair in ipairs(char_combinations) do
    ...
    repeat
      ...
      p1 = antinode1
      p2 = antinode2
    until (antinode1.y < 1 or antinode1.y > grid.h or antinode1.x < 1 or antinode1.x > grid.w) and
          (antinode2.y < 1 or antinode2.y > grid.h or antinode2.x < 1 or antinode2.x > grid.w)
  end

  -- For each antenna, add the antenna as an antinode.
  for x = 1, grid.w do
    for y = 1, grid.h do
      local node = grid.grid[y][x]
      if node and node.value ~= "." then
        antinode_grid:Insert(y, x, 1)
      end
    end
  end
