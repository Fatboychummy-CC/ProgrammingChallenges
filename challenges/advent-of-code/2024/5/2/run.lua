...
--- Fix the order of an update.
---@param update aoc.2024.5.1.Update The update to fix.
---@param rule_maps table<integer, aoc.2024.5.1.RuleMap> The rules to check against.
---@return integer middle The middle value of the fixed update.
local function fix_update(update, rule_maps)
  table.sort(update.list, function(a, b)
    if rule_maps[a] and rule_maps[a].after_map[b] then
      return true
    end

    if rule_maps[b] and rule_maps[b].before_map[a] then
      return true
    end
  ...
  return update.list[math.ceil(#update.list / 2)]
end
...
local function run(input, output)
  ...
  -- Part 2: For each INCORRECTLY ordered update, fix the order and sum the middle value.
  ...
  for _, update in ipairs(updates) do
    if not check_update(update, rule_maps) then
      -- Fix the order of the update.
      sum = sum + fix_update(update, rule_maps)
    end
  end
