...
local function can_determine(n, numbers)
  ...
  local function insert(obj, v)
    obj.new_child(obj.value + v)
    obj.new_child(obj.value * v)
    obj.new_child(
      tonumber(tostring(obj.value) .. tostring(v))
    )
  end
...
local function run(input, output)
  ...
  -- Part 2: Concatenation operator is now also used. `12 .. 34` is `1234`.

  local sum = 0

  local n_lines = #input:readLines()
  input:seek("set", 1)
  ...
  for line in input:lines() do
    ...
    term.clearLine()
    term.write("Progress: " .. math.floor((i / n_lines) * 100) .. "%")

    os.queueEvent("h")
    os.pullEvent("h")
  end

  output:write(("%.0f"):format(sum))
end
